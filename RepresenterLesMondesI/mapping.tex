...
\begin{quoting}[vskip=1ex,leftmargin=5mm,rightmargin=5mm]\sf
  In a map from the early memoirs of the Acad\'emie des Sciences,
  vol. VII, p. 430, one of the first in which longitudes were
  counted from the Paris Observatory, the shaded boundaries are those
  that result from the astronomical observations made, under the
  orders of Louis XIV, by various members of the Acad\'emie des
  Sciences; the boundaries in simple outline, with the names in
  ...
  Brittany was shifted more than 100 kilometers to the west. Thus
  Louis XIV, on seeing this map, complained to the academicians that
...
\begin{question}
  Aside from this operations of rotating the sheet of paper, shifting
  and rescaling a map, is there a unique optimal map?
\end{question}
...
\begin{figure}
  ...
  \rule{0pt}{1ex}
\end{figure}
The left figure above is a spherical cap $X$, centered for instance at the North Pole.
...
\begin{center}
  \includegraphics[width=0.7\textwidth]{Pix/AzEqHN}
\end{center}
In cartography, this map is called the \textit{azimuthal equidistant
  projection}%
\footnoteE{Check the flag of the UN, for an example. (Ed.)} %
(attributed to Guillaume Postel), while mathematically
speaking, this map  is called the (inverse of)
\textit{exponential map}.
...
equidistant map. The result is a horrible formula:
\[
...
\footnoteE{SNCF (\textbf{S}oci\'et\'e \textbf{n}ationale des
  \textbf{c}hemins de \textbf{f}er fran\c{c}ais) is French railway
  operator.} %
...
\footnoteE{Cartogram is a map where the area of geographic regions on
  the map is distorted to represent a specific quantity, such as
  population or GDP, instead of their actual land area. Of course,
  such maps are not suitable for navigation, but serve as
  visualization of other features, but distances.} %
...
follows from two theorems, due to Oxtoby–Ulam and Moser (in rather
different settings, but explaining that would require a bit of
mathematical background). I unfortunately cannot give a precise
statement here, but the general idea is as follows.

We are given a “measure” $m$ on $X$, which might represent a density
of population, of cars, of wealth, etc. Each subset $P$ of $X$ thus
has a certain measure $m(P)$, which is the number of inhabitants in
$P$, the number of cars, the total wealth of the inhabitants of $P$,
etc. We make almost no assumptions about this measure, and the theorem
asserts that there exists a map $f:X \to Y$ such that
$m(P) = \operatorname{area}(f(P))$ for every subset $P$.

Better yet, these theorems are “constructive,” meaning that one can
program a computer to draw the map, as in the examples above.
