...
how large this number for $\epsilon=0.01$ is. It has also been shown
that there are families of graphs for which $M=M(\epsilon)$ is indeed
about that big, so that the bound can not be improved by much. We may
still hope that such synthetic examples are rare and there are large
classes of graphs for which Szemer\'edi Regularity Lemma with some
reasonable $M$ holds.
}
