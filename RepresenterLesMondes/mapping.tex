...
different branches of the sciences.  Here is a map of the scientific
world, taken from%
...
The first idea is to consider two journals $i$ and $j$ are considered
to be thematically
close if the angle between their corresponding vectors is
small. Indeed, if two vectors point in almost the same direction, it means
they assign similar relative importance to all journals --- in other
words, they are interested in roughly the same topics.

% If you remember
% the dot product of two vectors, you know that one can easily compute
% the angle between two vectors from their coordinates. The same idea
% applies whether we are in two or three dimensions — or in 7,000!
% High-dimensional geometry should not frighten us. In any case, it is
% reasonable to accept that, one way or another, we can estimate
% “distances” between journals.
...
\footnoteE{\href{https://images-des-maths.pages.math.cnrs.fr/freeze/Representer-les-mondes.html}
  ...
  \url{https://images-des-maths.pages.math.cnrs.fr/freeze/Representer-les-mondes.html}. English
  translation: \href{???}{Mapping the Worlds I}, \url{???}} %
--- though, again, many other approaches could be used.
...
\footnoteE{Even relatively small graphs, e.g. with five vertices, may
  not fit on a paper in an embedded way, that is, some edges must
...
number of vertices so big that it defies imagination. Earlier, we saw
...
which information circulates, somewhat like a electrical engineer
...
\footnoteA{A recurring theme in the development of mathematics: the
  same idea can be usefull multiple times, often in unexpected
  ways.}
...
$Y$. We shall say that $A$ and $B$ are \textit{well connected} (to within
1\%) if this probability does not depend too much on the choice of
subsets $X\subset A$ and $Y\subset B$.
...
\red{[Remove this par]} Readers should not be intimidated by
...
$Y_{2}$​. For $A$ and $B$ to be well connected, there should be roughly
the same number of connections in both cases.



We will need anther definition.
We say that we have a partition of a graph when we have divided its
vertex set into subsets $A_{1},A_{2},\dots,A_{n},$ such that every
vertex of the graph belongs to exactly one of them.
...
journals into ``clusters'' that we called ``sciences.'' Here, for
instance, is a partition into 5 parts containing 2, 5, 4, 4, and 2
vertices.
\end{document}
