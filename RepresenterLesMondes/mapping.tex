...
\footnoteE{\href{https://images-des-maths.pages.math.cnrs.fr/freeze/Representer-les-mondes.html}
  ...
  English translation: \href{???}{Mapping the Worlds I}, \url{???}} %
...
known in the literature as Szemer\'edi's Regularity Lemma. Here we
give the original version. For a fixed precision level $\epsilon>0$
...
Now we are ready to state the theorem.  The placement of quantifiers
is somewhat tricky. First we choose the required precision level
$\epsilon$ ($1\%$ in the text above) and a minimal number of parts in
the partition. The theorem asserts that there is a maximum number of
parts such that for all sufficiently large graphs there is an
$\epsilon$-regular partition with number of parts squeezed between the
given minimum and maximum. The maximum number of parts and the
threshold of ``largness'' depend on the chosen precision level and
minimal number of parts.
\begin{theorem}[Szemer\'edi's Regularity Lemma]
  For any $\epsilon>0$ (precision level) and $m>0$ (minimal number of
  parts), there is $M>m$ (maximal number of parts) such that for every
  sufficiently large graph $G$ there exist an $\epsilon$-regular
  partition of $G$ with the number of parts in the interval $[m,M]$.
\end{theorem}
The known bounds on $M$ are just astounding, they are of the form of
exponential towers (repeated exponentiation) of the height
proportional to $\epsilon^{-2}$.%
\footnoteE{W.T. Gowers, \textit{Quasirandomness, counting and
    regularity for 3-uniform hypergraphs}, Combin. Probab. Comput. 15
  (2006), 143–184.} %
It has also been shown that there are graphs for which $M=M(\epsilon)$
is indeed about that big. We may still hope that such synthetic
examples are rare and there are large classes of graphs for which
Szemer\'edi Regularity Lemma with some reasonable $M$ holds.
...
actual, reasonable values of $n$ for realistic levels of precision. To
